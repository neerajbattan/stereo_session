...
def rectify (left, right, rmin=None):
  ...
  #detector = cv2.SURF(0.)
  detector = cv2.xfeatures2d.SURF_create(0.)

  #detector = cv2.ORB_create()
  kp1, desc1 = detector.detectAndCompute(uint8(left.mean(axis=2)).copy(), mask, False)
  kp2, desc2 = detector.detectAndCompute(uint8(right.mean(axis=2)).copy(), mask, False)
